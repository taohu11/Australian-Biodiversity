...
clc;
%%获得物种的列表
parentFolder = 'F:\TAO\Histrorical_impact\Annual-species-suitability_20-year_snapshots_5km\mammals';
subFolders = dir(parentFolder);
subFolders = subFolders(3:length(subFolders),1);  %因为前两个读出来是空的，如果需要的话得再看看
% subFolders = subFolders(474:length(subFolders),1);  %因为前两个读出来是空的，如果需要的话得再看看
...
for i = 1:length(subFolders)
    ...
    name_wenjianjia = ['F:\TAO\Histrorical_impact\Annual-species-suitability_20-year_snapshots_5km\mammals\',species,'\'];
    biodiversity_name = ['F:\TAO\Histrorical_impact\Annual-species-suitability_20-year_snapshots_5km\mammals\',species,'\',species,'_Historical_1970-2000_AUS_5km_ClimSuit.tif'];
    ...
    %%read boundary
    shp = shaperead(['F:\TAO\Histrorical_impact\Species-occurance-points\mammals\models\',species,'\mammals_',species,'_Extent_of_occurrence_buffered.shp']);
    ...
    %%输出tif
    geotiffwrite(['F:\TAO\Histrorical_impact\Quality_biodiversity\mammals\',species,'.tif'],quality_biodiversity,R,'GeoKeyDirectoryTag',info.GeoTIFFTags.GeoKeyDirectoryTag);
end
